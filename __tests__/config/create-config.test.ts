...
describe('create configuration in non-production environment', () => {
  ...
  const runClientSideTests = () => {
    it('creates default non-production configuration if process.browser === true', () => {
      ...
      expect(config.backend.loadPath).toEqual('/public/locales/{{lng}}/{{ns}}.json')
      expect(config.backend.addPath).toEqual('/public/locales/{{lng}}/{{ns}}.missing.json')
    })
    ...
    it('creates custom client-side non-production configuration', () => {
      ...
      expect(config.backend.loadPath).toEqual('/public/translations/{{ns}}/{{lng}}.json')
      expect(config.backend.addPath).toEqual('/public/translations/{{ns}}/{{lng}}.missing.json')
    })
    ...
    describe('localeExtension config option', () => {
      ...
      it('accepts any string and modifies backend paths', () => {
        ...
        expect(config.backend.loadPath).toEqual('/public/translations/{{ns}}/{{lng}}.test-extension')
        expect(config.backend.addPath).toEqual('/public/translations/{{ns}}/{{lng}}.missing.test-extension')
      })
    })
  }
