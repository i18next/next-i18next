...
2. Create a `next.config.js` file inside your root directory if you want to use locale subpaths. You can see this approach in the [examples/simple/next.config.js](./examples/simple/next.config.js) **make sure you have next.js version 9.5 or newer**.
...
const SomeLink = () => (
  <Link href="/some-page">This will magically prepend locale subpaths</Link>
);
```
