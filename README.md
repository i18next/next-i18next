...
class Footer extends React.Component {
  render() {
    return <footer>{this.props.t("description")}</footer>;
  }
}
...
If you use custom Next server directory https://github.com/zeit/next.js/#custom-server-and-routing add option `nextServerDir` with same value as is `dir`.
