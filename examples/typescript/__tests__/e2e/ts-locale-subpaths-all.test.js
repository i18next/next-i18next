...
describe('example project with localeSubpaths set to all', () => {
  beforeAll(async () => {
    ...
    await page.goto(`http://localhost:${jestPuppeteerConfig.e2eTests.TS_LOCALE_SUBPATHS_ALL.port}/`)
    await page.waitForSelector('html')
    await expect(page.url()).toBe(`http://localhost:${jestPuppeteerConfig.e2eTests.TS_LOCALE_SUBPATHS_ALL.port}/en/`)
  })
  ...
  describe(`changing to ${testLanguageSecondary} locale`, () => {
    beforeAll(async () => {
      ...
      await expect(page.url()).toBe(`http://localhost:${jestPuppeteerConfig.e2eTests.TS_LOCALE_SUBPATHS_ALL.port}/${testLanguageSecondary}/`)
    })
